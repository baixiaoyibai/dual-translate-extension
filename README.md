...
> **版本变更历史请见 [CHANGELOG.md](./CHANGELOG.md)**，当前版本：**v1.3.3**
...
- **一眼看状态**：翻译关闭时，扩展图标上会显示 ⏸ 暂停标记，无需点开弹窗确认
...
**翻译报错了怎么办？**
- 错误提示已用大白话说明原因（如「密钥校验失败」「网络超时」），按提示处理即可
- 页面顶部的错误横幅带「重试」按钮；未配置 API 时带「打开设置」按钮
- 单段翻译失败时，直接点那段文字即可重译该段
- 仍找不到原因？点弹窗里的「故障排查」，用一键全面检查自助诊断

## 安全性

- **API 密钥存在本地**（`chrome.storage.local`），**不会跨设备同步**到 Google 账户
- **密钥 PIN 码保护**：设置页所有密钥默认掩码显示，需验证 6 位 PIN 码后才能查看 / 编辑 / 复制；PIN 码使用 PBKDF2-HMAC-SHA256（10 万次迭代）+ 盐值哈希存储；页面刷新后自动锁定
...
- **存储**：`chrome.storage.sync` 存用户配置（不含密钥与术语表），`chrome.storage.local` 存 API 密钥、术语表、翻译缓存、API 状态。术语表自 v1.3.3 起存储于 local：受 `chrome.storage.sync` 单项 8KB 配额限制（`unlimitedStorage` 权限仅对 local 生效），约 11KB 的默认术语表在 sync 上无法写入；迁移到 local 后不再跨设备同步，如需跨设备迁移请使用设置页的导出/导入
...
- **密钥安全**：所有 API 密钥（含自定义供应商）存储在 `chrome.storage.local`，**不随 sync 同步**；`saveSettings` 合并而非覆盖密钥，防止单次保存丢失其他 API 配置；v1.0.13 新增 PIN 码保护（v1.2.16 起升级为 PBKDF2-HMAC-SHA256 慢哈希 + 盐值），设置页密钥掩码显示，导出时自动清除所有密钥
...
- **安全**：PIN 码保护（当时为 SHA-256 + 盐值哈希，v1.2.16 起已升级为 PBKDF2-HMAC-SHA256），密钥掩码显示，导出时清除自定义供应商密钥
