...
class VolcanoTranslator {
  ...
  async _buildAuthHeaders(bodyStr) {
    ...
    // 派生签名密钥
    // 注意：火山引擎 V4 签名中，每一步 HMAC 的密钥使用上一步结果的十六进制字符串
    const kDate = await this._hmac(this.secretKey, dateStamp);
    const kRegion = await this._hmac(this._toHex(kDate), this.region);
    const kService = await this._hmac(this._toHex(kRegion), this.service);
    const kSigning = await this._hmac(this._toHex(kService), 'request');

    const signature = this._toHex(
      await this._hmac(this._toHex(kSigning), stringToSign)
    );
