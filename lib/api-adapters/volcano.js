...
class VolcanoTranslator extends BaseTranslator {
  constructor(config) {
    ...
    // v1.2.3 fix: 从 endpoint 解析 host，避免用户配置自定义接入点时 V4 签名 host 不匹配导致认证失败
    try {
      this.host = new URL(this.endpoint).host || 'translate.volcengineapi.com';
    } catch {
  ...
  async translate(texts, sourceLang, targetLang, signal) {
    ...
    // H10: 翻译结果条数与分块数不匹配时记录警告
    if (allTranslations.length !== chunkMap.length) {
      console.warn(`[volcano] 翻译结果条数(${allTranslations.length})与分块数(${chunkMap.length})不匹配`);
    }
    for (let i = 0; i < allTranslations.length; i++) {
      ...
      // v1.0.7 fix: 合并拆分翻译时插入换行分隔符，避免丢失段落边界
      const translation = allTranslations[i] || '';
      if (merged[meta.originalIndex]) {
        merged[meta.originalIndex] += '\n' + translation;
      } else {
        merged[meta.originalIndex] = translation;
      }
    }
  ...
  async _translateBatch(texts, sourceLang, targetLang, signal) {
    ...
    const url = `${endpoint}/?Action=TranslateText&Version=2020-06-01`;
    ...
    if (data.ResponseMetadata && data.ResponseMetadata.Error) {
      ...
      // 限流相关错误
      // v1.2.12 fix: X-1 — 频率限制改为临时性 RATE_LIMITED，60s 内自动恢复而非等到日/月重置
    ...
    return data.TranslationList.map(item => (item && item.Translation) || '');
  }
