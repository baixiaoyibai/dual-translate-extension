...
class ApiManager {
  ...
  async _loadLlmPrompt() {
    ...
    if (this._llmPromptCached != null) return this._llmPromptCached;
    try {
    ...
    for (const { name, translator } of ordered) {
      ...
      try {
        ...
        if (this.statusCache[name]?.status !== 'available') {
          const updatedStatus = await settingsManager.saveApiStatus(name, { status: 'available' });
  ...
  async testApi(apiName, apiConfig) {
    ...
    try {
      ...
      const updatedStatus = await settingsManager.saveApiStatus(apiName, { status: 'available' });
