...
class ApiManager {
    ...
    for (const { name, translator } of ordered) {
      ...
      try {
        ...
        if (this.statusCache[name]?.status !== 'available') {
          ...
          const updatedStatus = await settingsManager.saveApiStatus(name, { status: 'available', consecutiveErrors: 0, reason: '' });
          ...
          if (this.statusCache[name]?.reason) {
            const updatedStatus = await settingsManager.saveApiStatus(name, { reason: '' });
            this.statusCache[name] = updatedStatus[name] || { status: 'available' };
          }
        }
        return results;
  ...
  async _translateWithTimeout(translator, texts, sourceLang, targetLang, apiName) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      try {
        ...
        if (msg === 'QUOTA_EXCEEDED' || msg.startsWith('AUTH_ERROR')) {
          throw error;
        }
  ...
  async testApi(apiName, apiConfig) {
    ...
    try {
      ...
      const updatedStatus = await settingsManager.saveApiStatus(apiName, { status: 'available', consecutiveErrors: 0, reason: '' });
