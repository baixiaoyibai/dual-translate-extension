...
class ApiManager {
  ...
  _buildTranslators() {
    ...
    for (const apiName of settings.api.apiPriority) {
      if (apiName === 'tencent') continue;
      if (!enabledApis[apiName]) continue;
      if (!apiKeys[apiName]) continue;
  ...
  async translate(texts, sourceLang, targetLang) {
    ...
    if (allExhausted) {
      const settings = settingsManager.settings;
      const retryMinutes = settings.advanced.retryInterval;
      throw new Error(`所有翻译服务暂时不可用，将在 ${retryMinutes} 分钟后重试`);
    }
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        const result = await Promise.race([
          translator.translate(texts, sourceLang, targetLang),
...
export { apiManager, LLM_PROVIDERS };