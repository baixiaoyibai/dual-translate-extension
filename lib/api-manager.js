...
import { TencentTranslator } from './api-adapters/tencent.js';
...
class ApiManager {
  ...
  _buildTranslators() {
    ...
    for (const apiName of settings.api.apiPriority) {
      ...
      if (apiName === 'baidu') {
        ...
        if (t.isConfigured()) this.translators.set(apiName, t);
      } else if (apiName === 'tencent') {
        const t = new TencentTranslator({
          secretId: apiKeys[apiName]?.secretId || '',
          secretKey: apiKeys[apiName]?.secretKey || '',
          region: apiKeys[apiName]?.region || 'ap-guangzhou'
        });
        if (t.isConfigured()) this.translators.set(apiName, t);
  ...
  async testApi(apiName, apiConfig) {
    ...
    if (apiName === 'baidu') {
      ...
      translator = new TencentTranslator({
        secretId: apiConfig.secretId || '',
        secretKey: apiConfig.secretKey || '',
        region: apiConfig.region || 'ap-guangzhou'
      });
    } else if (LLM_PROVIDERS[apiName]) {
