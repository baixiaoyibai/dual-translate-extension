...
// v1.3.0 fix P1-1: 一般性错误连续 3 次后触发 5 分钟冷却，到期自动恢复可用（对齐 rate_limited 冷却语义）
...
class ApiManager {
  ...
  _isApiUsable(apiName) {
    ...
    if (status.status === 'error' && status.consecutiveErrors >= 3) {
      // v1.3.0 fix P1-1: 连续 3 次一般性错误进入 5 分钟冷却，到期后自动恢复可用（避免 provider 被永久禁用、无恢复途径）
      if (!status.cooldownUntil || Date.now() >= status.cooldownUntil) return true;
      return false;
    }
    return true;
  ...
  async _translateWithTimeout(translator, texts, sourceLang, targetLang, apiName) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      try {
        ...
        // v1.2.15 fix: AbortError (timeout) should not be retried -- throw immediately
        // so the outer translate() loop can switch to the next API provider
        if (error && (error.name === 'AbortError' || msg === 'AbortError' || msg.includes('aborted'))) {
          throw error;
        }
        if (attempt < maxRetries) {
          // v1.2.15 fix: retryInterval is in seconds (not minutes) to avoid blocking
          // the entire translation pipeline for 5+ minutes on a transient error
  ...
  async _handleApiError(apiName, error) {
    ...
    if (msg === 'QUOTA_EXCEEDED') {
      ...
      // v1.3.2 fix N1: 上次 5 分钟冷却到期恢复后，失败计数从 1 重新起算，避免「连续 3 次」退化为「累计 3 次」
      const cooldownExpired = current.status === 'error' && current.cooldownUntil && Date.now() >= current.cooldownUntil;
      const consecutiveErrors = cooldownExpired ? 1 : ((current.consecutiveErrors || 0) + 1);
      // v1.3.0 fix P1-1: 连续 3 次一般性错误后写入 cooldownUntil，5 分钟冷却到期自动恢复可用
      const cooldownUntil = consecutiveErrors >= 3 ? Date.now() + ERROR_COOLDOWN_MS : (cooldownExpired ? 0 : (current.cooldownUntil || 0));
      allStatus = await settingsManager.saveApiStatus(apiName, {
        status: 'error',
        reason: msg,
        consecutiveErrors,
  ...
  getApiStatusSummary() {
    ...
    for (const apiName of priority) {
      ...
      let status = cachedStatus?.status || (isBuilt ? 'available' : 'unconfigured');
      let reason = cachedStatus?.reason || '';
      // v1.3.2 fix P3-1: 对可自动恢复类状态（额度重置/限流冷却/错误冷却）复用 _isApiUsable 的过期判定，
      // 到期后归一为 available，避免 popup 状态滞后、甚至与 getAvailableCount 互相矛盾
      if (cachedStatus && status !== 'available' && status !== 'auth_error' && status !== 'unconfigured' && this._isApiUsable(apiName)) {
        status = 'available';
        reason = '';
      }
      summary[apiName] = {
