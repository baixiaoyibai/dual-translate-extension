...
class SettingsManager {
  ...
  async resetApiQuotaIfNeeded() {
    ...
    // v1.0.10 fix: 月份格式统一为 ISO YYYY-MM (UTC)，与 getMonthlyUsage 保持一致
    // 之前用 local time 且无零填充 ("2026-7")，与 getMonthlyUsage 的 "2026-07" 不匹配
  ...
  async verifyPin(pin) {
    ...
    return {
      success: hash === result[PIN_HASH_KEY],
      error: hash === result[PIN_HASH_KEY] ? undefined : 'PIN 错误'
    };
  }
