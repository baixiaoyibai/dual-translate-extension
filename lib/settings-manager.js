...
class SettingsManager {
  ...
  async applyImportedSettings(importedSettings) {
    ...
    if (cleaned.api && cleaned.api.apiEndpoints && typeof cleaned.api.apiEndpoints === 'object') {
      for (const [name, endpoint] of Object.entries(cleaned.api.apiEndpoints)) {
        if (typeof endpoint !== 'string' || (endpoint && !/^https:\/\//i.test(endpoint) && !/^https?:\/\/localhost(?::\d+)?(?:\/|$)/i.test(endpoint))) {
          throw new Error(`导入的 API 接口地址不安全: ${name}`);
        }
      }
    }
