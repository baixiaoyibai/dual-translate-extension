...
class SettingsManager {
  ...
  // v1.0.6 fix: 强制从 local storage 重新加载 apiKeys，确保 getSettings 返回的密钥不丢失
  // 场景：SW 重启后 settingsManager.settings 可能未正确合并 apiKeys，
  //       导致设置页 API 卡片密钥为空，但 popup（走 apiManager.reload）仍正常
  async reloadApiKeys() {
  ...
  _mergeKeysIntoApi(keysData) {
    // v1.0.6 fix: local storage 是 apiKeys 的唯一权威来源，强制覆盖
    // 旧逻辑仅填充空字段（if (!current || current.length === 0)），
    // 当 sync storage 残留了旧的非空密钥时，local 中的新密钥无法合并进来，
    // 导致设置页显示旧值或空值
    if (!this.settings.api.apiKeys) {
      this.settings.api.apiKeys = {};
    }
  ...
  async saveSettings(settings) {
    ...
    if (syncData.api && syncData.api.apiKeys) {
      ...
      try {
        // v1.0.6 safety: 如果所有密钥字段都为空，跳过写入，防止覆盖 local storage 中的真实密钥
        // 场景：设置页未正确加载密钥时，saveAllSettings 会发送全空的 apiKeys，
        //       若直接覆盖 local storage，真实密钥将永久丢失
        ...
        if (hasAnyNonEmpty) {
          ...
          console.warn('[settings-manager] saveSettings: 跳过写入全空的 apiKeys（保护现有密钥）');
        }
      } catch (error) {
  ...
  async applyImportedSettings(importedSettings) {
    ...
    await chrome.storage.sync.set({ [SETTINGS_KEY]: importedSettings });
  ...
  async saveApiStatus(apiName, status, existingAllStatus = null) {
    const allStatus = existingAllStatus ? { ...existingAllStatus } : await this.getApiStatus();
    allStatus[apiName] = {
      ...status,
      updatedAt: Date.now()
    };
  ...
  async resetApiQuotaIfNeeded() {
    ...
    const batch = await chrome.storage.local.get([API_STATUS_KEY, RESET_DATE_KEY, RESET_MONTH_KEY]);
    let allStatus = batch[API_STATUS_KEY] || {};
    let changed = false;

    const dateResult = { [RESET_DATE_KEY]: batch[RESET_DATE_KEY] };
    ...
    const monthResult = { [RESET_MONTH_KEY]: batch[RESET_MONTH_KEY] };
